...
| 2.5 | Password policy | Min 8 chars, at least 1 uppercase, 1 number (enforced by Joi schema) |
| 2.6 | Google OAuth validation | Verify `id_token` with Google's public keys, check `aud` matches client ID |
| 2.7 | Role hierarchy enforcement | `owner > admin > agent > viewer`; middleware checks minimum role per route |
| 2.8 | Token refresh flow | Only valid (non-expired) refresh tokens accepted; old refresh token invalidated on use |
| 2.9 | Login brute-force protection | 5 failed attempts → 15-min lockout per email (stored in Redis or in-memory) |
