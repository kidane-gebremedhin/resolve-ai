// Platform-admin organizations page. Server component: lists all orgs with
// aggregated member / conversation / KB counts from /admin/organizations.
...
async function loadOrgs(): Promise<{ orgs: AdminOrg[]; error: string | null }> {
  try {
    const { items } = await api.get<{ items: AdminOrg[]; nextCursor: string | null }>(
      "/admin/organizations?limit=100",
    );
    return { orgs: items, error: null };
  } catch (err) {
    const message = err instanceof ApiError ? err.message : "Failed to load organizations";
    return { orgs: [], error: message };
  }
}

export default async function AdminOrganizationsPage() {
  const { orgs, error } = await loadOrgs();
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {formatNumber(orgs.length)} loaded · across all tenants
          </p>
        </div>
      </div>
              ...
              <tr key={o._id} className="hover:bg-muted/30">
                ...
                <td className="px-4 py-2.5">
                  ...
                  {o.subscriptionStatus ? (
                    <span className="ml-1 text-xs text-muted-foreground">{o.subscriptionStatus}</span>
                  ) : null}
                </td>
