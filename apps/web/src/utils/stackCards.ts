...
export class StackCards {
  ...
  private setStackCards(): void {
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      if (isNaN(this.marginY)) item.style.transform = 'none';
      else item.style.transform = 'translateY(' + this.marginY * i + 'px)';
    }
  }
  ...
  private animateStackCards(): void {
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      if (scrolling > 0) {
        ...
        (this.items[i] as HTMLElement).style.transform = 'translateY(' + this.marginY * i + 'px) scale(' + scaling + ')';
      } else {
...
export function initStackCards(): void {
  ...
  if (stackCards.length > 0 && intersectionObserverSupported && !reducedMotion) {
    ...
    let resizingId: ReturnType<typeof setTimeout> | false = false;
